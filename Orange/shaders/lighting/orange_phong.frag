...
// Texture maps
uniform sampler2D u_diffuseMap;   // GL_TEXTURE0 — orange colour
uniform sampler2D u_normalMap;    // GL_TEXTURE1 — tangent-space normals
uniform sampler2D u_aoMap;        // GL_TEXTURE2 — ambient occlusion (optional)
uniform sampler2D u_roughnessMap; // GL_TEXTURE3 — roughness (optional)
...
void main() {
	...
	// Directions in tangent space (already normalised by interpolation)
	...
	// --- Phong lighting with perturbed normal ---
	float NdotL = max(dot(normal, light_dir), 0.0);

	// Phong specular: reflect(-L, N) · V
