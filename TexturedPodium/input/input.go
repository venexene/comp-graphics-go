...
// затухания (Z/X/C/V, M), ambient (B/N), весов текстур (5/6/7/8), сброс (Ctrl+R).
package input
...
func ProcessInput(
	...
	if window.GetKey(glfw.Key5) == glfw.Press {
		...
		if lightCfg.MaterialWeight < 0.0 {
			lightCfg.MaterialWeight = 0.0
		}
	}
	if window.GetKey(glfw.Key6) == glfw.Press {
		lightCfg.MaterialWeight += ParamAdjustSpeed
		if lightCfg.MaterialWeight > 1.0 {
			lightCfg.MaterialWeight = 1.0
		}
	}
	if window.GetKey(glfw.Key7) == glfw.Press {
		lightCfg.NumberWeight -= ParamAdjustSpeed
		if lightCfg.NumberWeight < 0.0 {
			lightCfg.NumberWeight = 0.0
		}
	}
	if window.GetKey(glfw.Key8) == glfw.Press {
		lightCfg.NumberWeight += ParamAdjustSpeed
		if lightCfg.NumberWeight > 1.0 {
			lightCfg.NumberWeight = 1.0
		}
	}
