// Пакет scene: функции рендеринга — установка uniform-переменных,
// отрисовка пьедестала (DrawPodium). Содержит CreateWhiteTexture
// для создания текстуры-заглушки (не используется в финальной сцене).
package scene
...
// CreateWhiteTexture создаёт текстуру 1×1 белого пикселя (RGBA 255,255,255,255).
// Используется как заглушка для нетестурированных моделей. 
// Формат: GL_RGBA, фильтр LINEAR, обёртка REPEAT.
...
// setTransformUniforms загружает матрицы Model, View, Projection и normal_mat.
// Model — локальная→мировая (передаётся отдельно для каждого кубика).
// View — видовая (камера), Projection — перспективная проекция (общие).
// normal_mat = (Model⁻¹)ᵀ, используется для преобразования нормалей.
...
// setMaterialUniforms загружает свойства материала в шейдер.
// Uniform-структура: struct Material { vec3 ambient; vec3 diffuse; vec3 specular; float sheen_coef; }
...
// setLightUniforms загружает параметры источника света.
// Uniform-структура: struct PointLight { vec3 ambient, diffuse, specular, position; float constant, linear, quadratic; float ambient_strength; }
// Дополнительные отдельные uniform: linear_coef, quadratic_coef, attenuation_mode.
...
// drawObject — вспомогательная функция для отрисовки одной модели
// с установкой transform-, material- и light-uniform. Не устанавливает
// текстурные uniform (используется для нетестурированных моделей).
func drawObject(model *objects.Model, modelMat mgl32.Mat4, view, proj mgl32.Mat4,
	...
	if u.ViewPos != -1 {
		// Устанавливается один раз до цикла, но переустановка безопасна.
	}

	model.Draw()
}

// setBlendUniforms загружает параметры мультитекстурирования.
// Привязывает текстуру материала к GL_TEXTURE0, текстуру номера к GL_TEXTURE1.
// Устанавливает u_cubeColor, u_materialWeight, u_numberWeight.
...
// DrawPodium выполняет полный рендер одного кадра сцены.
// 1. Очищает COLOR и DEPTH буферы.
// 2. Активирует шейдерную программу, кеширует uniform-локации.
// 3. Для каждого из 4 кубиков: устанавливает матрицу Model, материал,
//    свет, blend-параметры (текстуры + цвет), вызывает DrawArrays.
// 4. Если установлено сердце (Heart != nil): устанавливает его модель
//    на высоте нижнего центрального кубика, рендерит с materialWeight=1.
func DrawPodium(
	program uint32,
	p *Podium,
	cam *Camera,
	...
	// Рендер сердца: только материал (onyx) x красный цвет.
	if p.Heart != nil {
